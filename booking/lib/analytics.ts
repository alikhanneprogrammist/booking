...
// Фильтрацию по периоду делает вью (через lib/calendar); сюда приходит уже отобранный список.
// CANCELLED исключается вызывающим кодом ВЕЗДЕ, кроме разбивки по статусам (byEnum 'status').
